...
const renderCheapDay = cheapTicket => {
  ...
  cheapestTicket.innerHTML = "<h2>Самые дешевые билеты на выбраную даты</h2>";
...
formSearch.addEventListener("submit", event => {
  ...
  if (formData.from && formData.to) {
    const requestData = `?depart_date=${formData.when}&origin=${formData.from.code}&destination=${formData.to.code}`;
